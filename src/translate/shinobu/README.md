# server/src/translate/shinobu/ — Node-hosted copy of the browser Shinobu pipeline
...
the pipeline is derived from ShinobuTranslator, GPL-3.0; this repo is AGPL-3.0).
...
| `runtime/onnxBridge.js` | **Replaced** — re-exports the Node ONNX bridge `server/src/pipeline/nodeOnnxBridge.js`; converts pipeline TensorTransport `{data,dims,type}` ↔ onnxruntime-node `ort.Tensor` |
| `runtime/modelRegistry.js` | **Replaced** — wraps `server/src/pipeline/nodeModelRegistry.js` (absolute paths, `['cpu']` runtime) + session cache dedup |
...
`server/src/pipeline/nodeOnnxBridge.js` gained an additive `getSessionInfo(sessionId)`
